      ...
      document.getElementById("filterForm").addEventListener("submit", async (e) => {
        ...
        try {
          const res = await fetch(`/api/rekap_lama?start=${startDate}&end=${endDate}&vendor=${vendorId}&kendaraan=${kendaraanId}&barang=${barang}`);
      ...
      document.getElementById("resetFilter").addEventListener("click", async () => {
        ...
        try {
          const res = await fetch(`/api/rekap_lama?start=${startDate}&end=${endDate}`);
          const data = await res.json();
          renderTable(data);
        } catch (error) {
          console.error("Error fetching data:", error);
