    ...
    async function loadOldOliOptions() {
        try {
            const res = await fetch("/api/oli_masuk");
    ...
    async function editOli(id) {
        try {
            const res = await fetch(`/api/oli_masuk/${id}`);
            if (!res.ok) {
...
- Keterangan: ${oli.keterangan || '-'}`;
            ...
            const deleteRes = await fetch(`/api/oli_masuk/${id}`, { method: "DELETE" });
